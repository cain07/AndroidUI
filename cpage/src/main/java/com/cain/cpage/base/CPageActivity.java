...
/**
 * 页面跳转都通过CPageActivity 嵌套Fragment来实现,动态替换fragment只需要指定相应的参数。 避免Activity 需要再manifest中注册的问题。
 * 1.管理应用中所有CPageActivity 实例。 2.管理CPageActivity 实例和fragment的跳转
 *
 * @author xuexiang
 * @since 2018/5/24 下午3:36
 */
public class CPageActivity extends AppCompatActivity implements CoreSwitcher {
    /**
     * 应用中所有CPageActivity的引用
     */
    private static List<WeakReference<CPageActivity>> sActivities = new ArrayList<>();
    /**
     * 当前activity的引用
     */
    private WeakReference<CPageActivity> mCurrentActivity = null;
    /**
     ...
     * ForResult 的fragment
     */
    private CPageFragment mFragmentForResult = null;
    /**
     * 请求码，必须大于等于0
     */
     ...
     * @return 栈顶Activity
     */
    public static CPageActivity getTopActivity() {
        if (sActivities != null) {
            int size = sActivities.size();
            if (size >= 1) {
                WeakReference<CPageActivity> ref = sActivities.get(size - 1);
                if (ref != null) {
                    return ref.get();
                }
            }
    ...
    protected String getPageName() {
        CPageFragment frg = getActiveFragment();
        if (frg != null) {
     ...
     * 结束activity，设置是否显示动画
     *
     * @param activity      CPageActivity对象
     * @param showAnimation 是否显示动画
     */
    private void finishActivity(CPageActivity activity, boolean showAnimation) {
        if (activity != null) {
            activity.finish();
            //从activity列表中移除当前实例
    ...
    public boolean isFragmentTop(String fragmentTag) {
        int size = sActivities.size();
        if (size > 0) {
            WeakReference<CPageActivity> reference = sActivities.get(size - 1);
            CPageActivity activity = reference.get();
    ...
    public boolean findPage(String pageName) {
        ...
        for (int j = size - 1; j >= 0; j--) {
            WeakReference<CPageActivity> reference = sActivities.get(j);
            if (reference != null) {
                CPageActivity activity = reference.get();
                if (activity == null) {
    ...
    public Fragment gotoPage(CoreSwitchBean page) {
        ...
        for (int i = size - 1; i >= 0; i--) {
            WeakReference<CPageActivity> ref = sActivities.get(i);
            if (ref != null) {
                CPageActivity item = ref.get();
                if (item == null) {
                    PageLog.d("item null");
                    continue;
                }
     ...
     * @return 是否弹出成功
     */
    protected boolean popFragmentInActivity(final String pageName, Bundle bundle, CPageActivity findActivity) {
        if (pageName == null || findActivity == null || findActivity.isFinishing()) {
            ...
            if (fragmentManager != null) {
                ...
                if (frg != null && frg instanceof CPageFragment) {
                    ...
                    ((CPageFragment) frg).onFragmentDataReset(bundle);
                    return true;
                }
            }
        }
     ...
     */
    @Override
    public Fragment openPageForResult(CoreSwitchBean page, CPageFragment fragment) {
        if (page != null) {
            if (page.isNewActivity()) {
                ...
                CPageFragment frg = CorePageManager.getInstance().openPageWithNewFragmentManager(getSupportFragmentManager(), pageName, bundle, animations, addToBackStack);
                if (frg == null) {
                    return null;
                }
                final CPageFragment opener = fragment;
                frg.setRequestCode(page.getRequestCode());
                frg.setFragmentFinishListener(new CPageFragment.OnFragmentFinishListener() {
                    @Override
     ...
     * @return 打开的fragment对象
     */
    public <T extends CPageFragment> T openPage(Class<T> clazz) {
     ...
     * @return 打开的fragment对象
     */
    public <T extends CPageFragment> T openPage(Class<T> clazz, Bundle bundle) {
     ...
     * @return 打开的fragment对象
     */
    public <T extends CPageFragment> T changePage(Class<T> clazz) {
     ...
     * @return 打开的fragment对象
     */
    public <T extends CPageFragment> T changePage(Class<T> clazz, Bundle bundle) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        CPageFragment activeFragment = getActiveFragment();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        PageLog.d("onActivityResult from CPageActivity" + requestCode + " " + resultCode);
     ...
     */
    public CPageFragment getActiveFragment() {
        if (this.isFinishing()) {
            return null;
        }
        ...
        if (manager != null) {
            ...
            if (count > 0) {
                ...
                return (CPageFragment) manager.findFragmentByTag(tag);
            }
        }
        return null;
    }
    ...
    /**
     * 根据页面的类获取CPageFragment
     *
     * @return 获取的页面
     */
    public <T extends CPageFragment> T getPage(Class<T> clazz) {
        String pageName = PageConfig.getPageInfo(clazz).getName();
        CPageFragment fragment = getPageByName(pageName);
        if (fragment != null) {
            return (T) fragment;
        } else {
            return null;
    ...
    /**
     * 根据页面的名称获取CPageFragment
     *
     * @return 获取的页面
     */
    public CPageFragment getPageByName(String pageName) {
        if (this.isFinishing() || TextUtils.isEmpty(pageName)) {
            return null;
        }
        FragmentManager manager = this.getSupportFragmentManager();
        if (manager != null) {
            return (CPageFragment) manager.findFragmentByTag(pageName);
        }
        return null;
    }
    ...
    protected void printAllActivities() {
        PageLog.d("------------CPageActivity print all------------activities size:" + sActivities.size());
        for (WeakReference<CPageActivity> ref : sActivities) {
            if (ref != null) {
                CPageActivity item = ref.get();
                if (item != null) {
            ...
            if (page != null) {
                ...
                if (fragment != null) {
                    if (startActivityForResult) {
                        ...
                        fragment.setFragmentFinishListener(new CPageFragment.OnFragmentFinishListener() {
                            @Override
                            public void onFragmentResult(int requestCode, int resultCode, Intent intent) {
                                CPageActivity.this.setResult(resultCode, intent);
                            }
                        });
                    }
                } else {
